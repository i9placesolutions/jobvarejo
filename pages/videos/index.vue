...
onMounted(async()=>{try{await refreshLibrary();const selected=String(route.query.project||'');if(selected)await openVideo(selected)}catch(e){sayError(e)}finally{loading.value=false}poll=setInterval(()=>{if(view.value!=='editor')return;refreshHealth();if(projectId.value&&!liveJobs.value)refreshJobs().catch(()=>{})},6000)})

...
import {
  ...
  Mic2, MonitorPlay, Pause, Play, Plus, Radio, RefreshCw, Search, Send, Settings2, ShieldCheck, Store,
  Sparkles, UserPlus, Users, Volume2, X, Zap
...
const activeView = ref<'home' | 'catalog' | 'programs' | 'agenda' | 'requests' | 'team'>('home')
const query = ref('')
...
const requestForm = reactive({ kind: 'jingle', title: '', brief: '', style: '', lyrics: '', voiceId: '', gender: 'female' })
...
const artists = computed(() => ['Todos', ...Array.from(new Set(radio.catalog.value.map((track) => track.artist).filter(Boolean)))] )
const filteredCatalog = computed(() => radio.catalog.value.filter((track) => {
  ...
  return matchesText && matchesGenre && matchesArtist
}))
...
const musicGpt = computed(() => radio.bootstrap.value?.musicGpt || { configured: false, ttsConfigured: false })
...
const setView = (view: string) => {
  if (['home', 'catalog', 'programs', 'agenda', 'requests', 'team'].includes(view)) activeView.value = view as typeof activeView.value
}

const loadAll = async () => {
  try {
    await radio.loadBootstrap()
    await Promise.all([radio.loadCatalog(), radio.loadPlayer(), radio.loadRequests(), radio.loadMembers()])
...
const refreshCatalog = async () => {
  try {
    await radio.loadCatalog({ q: query.value || undefined, genre: activeGenre.value === 'Todos' ? undefined : activeGenre.value, artist: activeArtist.value === 'Todos' ? undefined : activeArtist.value })
...
const submitRequest = async () => {
  ...
  try {
    ...
    showNotice(result?.provider?.configured ? 'Solicitação enviada ao MusicGPT.' : 'Solicitação guardada. Configure o MusicGPT para envio automático.', result?.provider?.configured ? 'success' : 'info')
...
const refreshRequest = async (requestId: string) => {
  try {
    await $fetch('/api/radio-indoor/ai/refresh', { method: 'POST', body: { requestId } })
    await radio.loadRequests()
    showNotice('Status da solicitação atualizado.', 'success')
  } catch (error: any) { showNotice(error?.data?.statusMessage || 'Não foi possível consultar o MusicGPT', 'error') }
}
  ...
  <div class="radio-app">
    <aside class="radio-sidebar" :class="{ 'is-open': showMobileNav }">
      ...
      <nav class="main-nav" aria-label="Rádio Indoor">
        <button v-for="item in [
          ...
          { id: 'requests', label: 'Solicitar áudio', icon: Sparkles },
    ...
    <main class="radio-main">
      ...
      <template v-else-if="activeView === 'catalog'">
        ...
        <div class="filter-bar"><div class="search-box"><Search :size="17" /><input v-model="query" placeholder="Buscar música, artista ou álbum" @keyup.enter="refreshCatalog" /><button v-if="query" @click="query = ''; void refreshCatalog()"><X :size="14" /></button></div><select v-model="activeArtist" @change="refreshCatalog"><option v-for="artist in artists" :key="artist">{{ artist }}</option></select><select v-model="selectedPlaylistId" class="playlist-select"><option value="">Adicionar a…</option><option v-for="playlist in playlists" :key="playlist.id" :value="playlist.id">{{ playlist.name }}</option></select></div>
        <div class="chip-row"><button v-for="genre in genres" :key="genre" class="chip" :class="{ selected: activeGenre === genre }" @click="selectGenre(genre)">{{ genre }}</button></div>
        <div class="catalog-grid"><button v-for="track in filteredCatalog" :key="track.id" class="track-card" @click="playTrack(track)"><div class="cover"><img v-if="track.thumbnailUrl" :src="track.thumbnailUrl" :alt="track.title" loading="lazy" /><div v-else class="cover-placeholder"><Disc3 :size="38" /></div><span class="play-overlay"><Play :size="22" fill="currentColor" /></span><span class="track-add" title="Adicionar à playlist" @click.stop="addToPlaylist(track)"><Plus :size="16" /></span></div><div class="track-info"><strong>{{ track.title }}</strong><span>{{ track.artist }}</span><small>{{ track.genre }} <b>·</b> {{ track.releaseYear || 'Ano não informado' }}</small></div></button><div v-if="!filteredCatalog.length" class="empty-state"><Disc3 :size="30" /><strong>Nenhuma faixa encontrada</strong><p>Importe o catálogo enviado para o Wasabi ou ajuste os filtros.</p><button class="button primary" @click="importCatalog">Importar agora</button></div></div>
      </template>
      ...
      <template v-else-if="activeView === 'requests'">
        <section class="page-heading"><div><span class="eyebrow muted">MUSICGPT</span><h1>Peça um áudio para a rádio</h1><p>Descreva a ideia. A solicitação fica registrada e o retorno aparece aqui quando o provedor concluir.</p></div><div class="provider-state" :class="{ connected: musicGpt.configured }"><span></span>{{ musicGpt.configured ? 'MusicGPT conectado' : 'Integração aguardando chave' }}</div></section>
        <div class="workspace-grid"><section class="editor-card request-editor"><div class="card-title"><div><h3>Nova solicitação</h3><p>Jingle, off, locução ou música original</p></div><Sparkles :size="20" /></div><div class="kind-tabs"><button v-for="kind in [{ id: 'jingle', label: 'Jingle', icon: Zap }, { id: 'off', label: 'Off', icon: Mic2 }, { id: 'voice', label: 'Locução', icon: Headphones }, { id: 'music', label: 'Música', icon: Disc3 }]" :key="kind.id" :class="{ selected: requestForm.kind === kind.id }" @click="requestForm.kind = kind.id"><component :is="kind.icon" :size="16" />{{ kind.label }}</button></div><div class="form-grid"><label>Título<input v-model="requestForm.title" placeholder="Ex.: Oferta de fim de semana" /></label><label>Estilo<input v-model="requestForm.style" placeholder="Energético, sertanejo, jovem..." /></label><label class="full">Briefing<textarea v-model="requestForm.brief" rows="4" placeholder="Conte o que deve ser dito ou criado, público, duração e clima..."></textarea></label><label class="full">Texto/lyrics (opcional)<textarea v-model="requestForm.lyrics" rows="3" placeholder="Texto exato para locução ou letra"></textarea></label><label>ID da voz (opcional)<input v-model="requestForm.voiceId" placeholder="Configurado no MusicGPT" /></label><label v-if="requestForm.kind === 'off' || requestForm.kind === 'voice'">Gênero da voz<select v-model="requestForm.gender"><option value="female">Feminina</option><option value="male">Masculina</option></select></label></div><button class="button primary" :disabled="isSubmittingRequest" @click="submitRequest"><LoaderCircle v-if="isSubmittingRequest" class="spin" :size="16" /><Send v-else :size="16" /> Enviar solicitação</button><p v-if="!musicGpt.configured" class="inline-note"><Settings2 :size="14" /> O pedido será salvo agora e enviado assim que <code>MUSICGPT_API_KEY</code> estiver configurada.</p></section><section class="editor-card request-guide"><div class="guide-gradient"><Sparkles :size="24" /></div><h3>Uma fila para cada ideia</h3><p>O pedido nasce com status e histórico. Quando o webhook do MusicGPT entregar o áudio, ele fica pronto para entrar em uma playlist.</p><div class="guide-step"><b>1</b><span>Descreva o áudio</span></div><div class="guide-step"><b>2</b><span>MusicGPT processa</span></div><div class="guide-step"><b>3</b><span>Você escolhe onde tocar</span></div></section></div>
        <section class="list-card"><div class="card-title"><div><h3>Solicitações recentes</h3><p>Acompanhe o que está em produção</p></div><button class="icon-button" title="Atualizar" @click="radio.loadRequests"><RefreshCw :size="17" /></button></div><div v-if="radio.requests.value.length" class="request-list"><article v-for="item in radio.requests.value" :key="item.id" class="request-row"><div class="request-kind" :class="item.kind"><Sparkles :size="17" /></div><div class="request-main"><strong>{{ item.title }}</strong><span>{{ item.brief }}</span></div><span class="request-status" :class="item.status"><i></i>{{ item.status === 'ready' ? 'Pronto' : item.status === 'processing' ? 'Processando' : item.status === 'failed' ? 'Falhou' : 'Na fila' }}</span><button v-if="item.providerTaskId && item.status !== 'ready' && item.status !== 'failed'" class="request-refresh" title="Consultar MusicGPT" @click="refreshRequest(item.id)"><RefreshCw :size="14" /></button></article></div><div v-else class="empty-inline"><Sparkles :size="24" /> Suas solicitações aparecerão aqui.</div></section>
      </template>

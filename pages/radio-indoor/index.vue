  ...
  <div class="radio-app" :class="{ 'theme-light': radioTheme === 'light' }">
    ...
    <main class="radio-main">
      ...
      <template v-else-if="activeView === 'requests'">
        <section class="page-heading"><div><span class="eyebrow muted">MUSICGPT</span><h1>Gere áudio para a rádio</h1><p>Crie jingles, músicas, offs e locuções. A solicitação fica registrada e o retorno aparece aqui quando o provedor concluir.</p></div><div class="provider-state" :class="{ connected: musicGpt.configured }"><span></span>{{ musicGpt.configured ? 'MusicGPT conectado' : 'Integração aguardando chave' }}</div></section>

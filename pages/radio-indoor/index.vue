...
const elevenLabs = computed(() => radio.bootstrap.value?.elevenLabs || { configured: false })
...
const changeStation = async () => {
  ...
  showStationEditForm.value = false
...
const openStationEdit = () => {
  stationEditForm.name = String(station.value?.name || '')
  stationEditForm.timezone = String(station.value?.timezone || 'America/Sao_Paulo')
  showStationForm.value = false
  showStationEditForm.value = !showStationEditForm.value
}

const saveStationDetails = async () => {
  if (!stationEditForm.name.trim() || !canManageUsers.value) return
  try {
    await radio.create({ action: 'toggle_station', status: station.value?.status || 'draft', ...stationEditForm })
    showStationEditForm.value = false
    showNotice('Nome e fuso da loja atualizados.', 'success')
  } catch (error: any) {
    showNotice(error?.data?.statusMessage || 'Não foi possível atualizar a loja', 'error')
  }
}
...
const createSchedule = async () => {
  if (!scheduleForm.programId || !scheduleForm.daysOfWeek.length || isCreatingSchedule.value) return
  ...
  finally { isCreatingSchedule.value = false }
}
...
const submitRequest = async () => {
  ...
  try {
    ...
    const destination = 'ElevenLabs'
    const message = String(result?.provider?.message || (
      !configured
        ? `Solicitação guardada. Configure ${destination} para envio automático.`
        : accepted
          ? `Solicitação enviada à ${destination}.`
          : `Solicitação salva, mas ainda sem envio à ${destination}.`
    ))
    showNotice(message, accepted ? 'success' : configured ? 'info' : 'info')
    await radio.loadRequests()
  } catch (error: any) {
    showNotice(error?.data?.statusMessage || 'Falha ao criar solicitação', 'error')
    await radio.loadRequests().catch(() => undefined)
  }
...
const useVoiceInRequest = (voice: any) => {
  ...
  // O clone é usado somente no TTS de off e locução.
  if (requestForm.kind === 'jingle' || requestForm.kind === 'music') {
    requestForm.kind = 'off'
  }
  activeView.value = 'requests'
  showNotice(`Voz “${voice?.name || 'selecionada'}” selecionada para off/locução na ElevenLabs.`, 'success')
}
...
const refreshRequest = async (requestId: string, silent = false) => {
  try {
    ...
    if (!silent) showNotice(error?.data?.statusMessage || 'Não foi possível consultar a solicitação antiga', 'error')
    return null
  }
}
  ...
  <div class="radio-app" :class="{ 'theme-light': radioTheme === 'light' }">
    ...
    <main class="radio-main">
      ...
      <template v-else-if="activeView === 'agenda'">
        ...
        <div class="workspace-grid"><section class="editor-card"><div class="card-title"><div><h3>Novo horário</h3><p>Início e fim iguais criam uma grade contínua de 24 horas.</p></div><Clock3 :size="20" /></div><label>Programa<select v-model="scheduleForm.programId"><option value="">Selecione</option><option v-for="program in programs" :key="program.id" :value="program.id">{{ program.name }}</option></select></label><div class="time-grid"><label>Início<input v-model="scheduleForm.startTime" type="time" /></label><label>Fim<input v-model="scheduleForm.endTime" type="time" /></label></div><div class="days"><span>Repetir em</span><button v-for="day in [{ id: 0, label: 'D' }, { id: 1, label: 'S' }, { id: 2, label: 'T' }, { id: 3, label: 'Q' }, { id: 4, label: 'Q' }, { id: 5, label: 'S' }, { id: 6, label: 'S' }]" :key="day.id" :class="{ selected: scheduleForm.daysOfWeek.includes(day.id) }" @click="toggleDay(day.id)">{{ day.label }}</button></div><button class="button primary" @click="createSchedule"><CalendarClock :size="16" /> Publicar horário</button></section><section class="editor-card schedule-help"><div class="help-art"><CalendarClock :size="34" /></div><h3>Como funciona</h3><p>O player da loja usa a agenda no fuso da estação e fica em silêncio fora dos horários. O player interno pode tocar o catálogo para teste enquanto a loja está em rascunho.</p><div class="help-line"><Check :size="15" /> Pode pausar sem apagar</div><div class="help-line"><Check :size="15" /> Cache local das próximas faixas</div><div class="help-line"><Check :size="15" /> Histórico de reprodução</div></section></div>
      ...
      <template v-else-if="activeView === 'voices'">
        <section class="page-heading"><div><span class="eyebrow muted">BANCO DE VOZES</span><h1>Vozes do JobVarejo</h1><p>As vozes autorizadas são cadastradas pelo administrador e usadas na ElevenLabs para criar offs e locuções.</p></div><div class="provider-state" :class="{ connected: elevenLabs.configured }"><span></span>{{ elevenLabs.configured ? 'ElevenLabs configurada' : 'Integração aguardando chave' }}</div></section>
...
:global(body) { margin: 0; background: #f3f7fb; color: #172b45; font-family: 'Plus Jakarta Sans', 'Barlow', system-ui, sans-serif; }
:global(*) { box-sizing: border-box; }
.radio-app { min-height: 100vh; background: radial-gradient(circle at 20% -10%, rgba(49,114,185,.24), transparent 35%), #0b0b12; display: flex; padding-bottom: 104px; }
...
.brand-lockup { display:flex; align-items:center; gap: 11px; padding: 2px 10px 34px; }.brand-mark { width: 38px; height:38px; border-radius: 12px; display:grid; place-items:center; color:white; background: linear-gradient(135deg,#66aeff,#3886da); box-shadow:0 6px 25px rgba(71,153,242,.38); }.brand-lockup strong { display:block; font-size:15px; letter-spacing:-.02em; }.brand-lockup span { display:block; color:#86919c; font-size:10px; text-transform:uppercase; letter-spacing:.13em; margin-top:3px; }
...
.request-action.preview { color:#c2dfff; border-color:rgba(112,170,236,.3); background:rgba(74,145,224,.12); }
...
.request-preview { display:flex; align-items:center; gap:12px; margin:0 0 14px; padding:12px 14px; border:1px solid rgba(112,170,236,.28); border-radius:12px; background:linear-gradient(110deg,rgba(41,75,113,.25),rgba(20,26,31,.8)); }
...
.request-playlist-pick select { min-width:160px; max-width:220px; color:#d9d4e4; background:#161c22; border:1px solid rgba(255,255,255,.1); border-radius:7px; padding:7px 8px; font:10px inherit; }
...
.sr-only { position:absolute; width:1px; height:1px; padding:0; margin:-1px; overflow:hidden; clip:rect(0,0,0,0); white-space:nowrap; border:0; }.station-switcher { display:flex; align-items:center; gap:7px; min-width:0; color:#a2c8f3; padding:5px 7px 5px 9px; border:1px solid rgba(125,181,245,.24); background:rgba(70,135,207,.08); border-radius:9px; }.station-switcher select { max-width:190px; min-width:90px; color:#eeeaf8; background:transparent; border:0; outline:0; font:600 11px inherit; cursor:pointer; }.station-switcher option { color:#1c242c; background:#fff; }.station-add { width:24px; height:24px; display:grid; place-items:center; border:1px solid rgba(255,255,255,.13); border-radius:7px; background:rgba(255,255,255,.06); color:#c7dffa; cursor:pointer; }.station-add:hover { background:#5ca3f2; color:#fff; }.station-create-panel { display:flex; align-items:center; justify-content:space-between; gap:18px; margin:18px 0 -2px; padding:14px 16px; border:1px solid rgba(125,181,244,.24); border-radius:12px; background:linear-gradient(105deg,rgba(39,81,128,.28),rgba(19,25,30,.78)); }.station-create-copy { display:flex; align-items:center; gap:10px; color:#b0d0f4; min-width:220px; }.station-create-copy strong,.station-create-copy span { display:block; }.station-create-copy strong { color:#eee8fb; font-size:12px; }.station-create-copy span { color:#89939e; font-size:10px; margin-top:4px; }.station-create-fields { display:flex; align-items:center; justify-content:flex-end; gap:8px; flex:1; }.station-create-fields input { color:#e8e4f0; background:#161c22; border:1px solid rgba(255,255,255,.1); border-radius:8px; padding:10px 11px; font:12px inherit; outline:none; min-width:150px; }.station-create-fields .button { white-space:nowrap; margin:0; }
...
.radio-app.theme-light {
  color-scheme: light;
  color: #242e38;
  background: radial-gradient(circle at 18% -12%, rgba(91,154,224,.16), transparent 35%), #f5f6fb;
}
.radio-app.theme-light .radio-sidebar { background: rgba(255,255,255,.94); border-right-color: rgba(42,59,77,.1); box-shadow: 12px 0 32px rgba(43,62,83,.05); }
.radio-app.theme-light .brand-lockup strong { color:#242f3b; }
.radio-app.theme-light .brand-lockup span { color:#8b8799; }
.radio-app.theme-light .nav-item { color:#6b7580; }
.radio-app.theme-light .nav-item:hover { color:#314255; background:rgba(76,130,190,.08); }
.radio-app.theme-light .nav-item.active { color:#345070; background:linear-gradient(90deg,rgba(76,153,238,.16),rgba(76,153,238,.04)); box-shadow:inset 2px 0 #508fd5; }
.radio-app.theme-light .private-badge,.radio-app.theme-light .cache-badge { color:#78828c; border-color:rgba(42,59,77,.1); background:rgba(246,246,251,.8); }
.radio-app.theme-light .cache-badge.ready { color:#318e62; border-color:rgba(55,166,107,.22); background:rgba(55,166,107,.08); }
.radio-app.theme-light .topbar { border-bottom-color:rgba(42,59,77,.1); }
.radio-app.theme-light .crumb { color:#8b8698; }
.radio-app.theme-light .crumb b { color:#3b4754; }
.radio-app.theme-light .station-status { color:#727c86; }
.radio-app.theme-light .station-switcher { color:#4674a7; border-color:rgba(77,138,205,.2); background:rgba(76,153,238,.07); }
.radio-app.theme-light .station-switcher select { color:#3a5069; }
.radio-app.theme-light .station-switcher option { color:#262f38; background:#fff; }
.radio-app.theme-light .station-add { color:#4171a7; border-color:rgba(62,95,131,.15); background:rgba(255,255,255,.75); }
.radio-app.theme-light .station-add:hover { color:#fff; background:#5090d8; }
.radio-app.theme-light .icon-button { color:#6b7580; border-color:rgba(42,59,77,.14); background:rgba(255,255,255,.72); }
.radio-app.theme-light .icon-button:hover { color:#355477; border-color:rgba(77,138,205,.35); background:#fff; }
.radio-app.theme-light .theme-toggle { color:#507aa7; }
.radio-app .topbar-actions .theme-toggle { display:grid; }
.radio-app.theme-light .notice { border-color:rgba(61,85,111,.16); }
...
.radio-app.theme-light .notice.info { color:#4372a7; background:rgba(76,153,238,.09); border-color:rgba(76,153,238,.2); }
...
.radio-app.theme-light .hero-panel { border-color:rgba(75,115,160,.18); background:linear-gradient(108deg,#e9e1fb,#fbfaff 50%,#eaf7f5); box-shadow:0 18px 42px rgba(57,88,122,.11); }
.radio-app.theme-light .eyebrow { color:#457ab6; }
.radio-app.theme-light .eyebrow.muted { color:#848e98; }
.radio-app.theme-light .eyebrow-dot { background:#5c9ae0; box-shadow:0 0 0 5px rgba(92,154,224,.13); }
.radio-app.theme-light .hero-copy h1 { color:#24303d; }
.radio-app.theme-light .hero-copy h1 em { color:#457ab6; }
.radio-app.theme-light .hero-copy p { color:#5f6973; }
.radio-app.theme-light .hero-disc { color:rgba(77,126,180,.42); border-color:rgba(66,105,148,.2); box-shadow:0 0 50px rgba(87,149,218,.16); }
.radio-app.theme-light .disc-center { background:#5e99db; border-color:rgba(255,255,255,.75); }
.radio-app.theme-light .button.secondary { color:#4c5968; border-color:rgba(55,78,103,.18); background:rgba(255,255,255,.72); }
.radio-app.theme-light .button.secondary:hover { background:#fff; }
.radio-app.theme-light .button.ghost { color:#456c98; border-color:rgba(69,113,162,.25); background:rgba(255,255,255,.42); }
.radio-app.theme-light .button.ghost:hover { background:rgba(76,153,238,.1); }
.radio-app.theme-light .stat-card,.radio-app.theme-light .editor-card,.radio-app.theme-light .list-card { border-color:rgba(51,72,94,.12); background:rgba(255,255,255,.84); box-shadow:0 12px 28px rgba(46,69,93,.06); }
.radio-app.theme-light .stat-card span { color:#717a84; }
.radio-app.theme-light .stat-card strong { color:#26303b; }
.radio-app.theme-light .stat-card small { color:#918b9d; }
.radio-app.theme-light .stat-icon.purple { color:#5088c7; background:rgba(76,153,238,.12); }
...
.radio-app.theme-light .section-heading h2,.radio-app.theme-light .page-heading h1 { color:#28323d; }
.radio-app.theme-light .text-button { color:#497bb4; }
.radio-app.theme-light .genre-card { border-color:rgba(58,86,117,.14); background:linear-gradient(145deg,rgba(211,229,248,.92),rgba(247,246,252,.96)); color:#4c77a9; }
...
.radio-app.theme-light .genre-card:hover { border-color:rgba(76,153,238,.45); }
.radio-app.theme-light .genre-card strong { color:#303b47; }
.radio-app.theme-light .genre-card small { color:#8a8495; }
.radio-app.theme-light .track-mini { color:#29333e; border-color:rgba(52,74,98,.13); background:rgba(255,255,255,.8); }
.radio-app.theme-light .track-mini:hover { border-color:rgba(76,153,238,.35); background:#fff; }
.radio-app.theme-light .cover { color:#5088c7; background:linear-gradient(135deg,#d2e2f4,#f3effb); }
.radio-app.theme-light .track-mini strong,.radio-app.theme-light .track-info strong { color:#2d3742; }
.radio-app.theme-light .track-mini small,.radio-app.theme-light .track-info span { color:#717a84; }
.radio-app.theme-light .mini-play { color:#5289c6; }
.radio-app.theme-light .page-heading p { color:#6a727c; }
.radio-app.theme-light .search-box { color:#7c8690; border-color:rgba(49,69,91,.14); background:rgba(255,255,255,.88); }
.radio-app.theme-light .search-box input { color:#2a3540; }
.radio-app.theme-light .search-box input::placeholder { color:#9a94a5; }
.radio-app.theme-light .search-box button { color:#7a8490; }
.radio-app.theme-light .filter-bar select,.radio-app.theme-light .form-grid select,.radio-app.theme-light .form-grid input,.radio-app.theme-light .form-grid textarea,.radio-app.theme-light .editor-card > label select,.radio-app.theme-light .editor-card > label input,.radio-app.theme-light .station-create-fields input { color:#2a3540; background:#fff; border-color:rgba(49,69,91,.16); }
.radio-app.theme-light .filter-bar select option,.radio-app.theme-light .form-grid select option,.radio-app.theme-light .member-row select option { color:#2a3540; background:#fff; }
.radio-app.theme-light .chip { color:#717a84; border-color:rgba(49,69,91,.13); background:rgba(255,255,255,.65); }
.radio-app.theme-light .chip.selected,.radio-app.theme-light .chip:hover { color:#3b6798; border-color:rgba(76,153,238,.35); background:rgba(76,153,238,.11); }
.radio-app.theme-light .track-card { color:#2a3540; }
.radio-app.theme-light .track-card .cover { border-color:rgba(49,69,91,.1); background:linear-gradient(135deg,#d2e3f5,#f5f2fb); }
.radio-app.theme-light .play-overlay { color:#fff; background:rgba(42,73,108,.42); }
.radio-app.theme-light .track-add { color:#fff; background:rgba(55,85,120,.68); border-color:rgba(255,255,255,.6); }
.radio-app.theme-light .track-info small { color:#8b8595; }
.radio-app.theme-light .track-info b { color:#4c82c0; }
.radio-app.theme-light .empty-state,.radio-app.theme-light .empty-inline { color:#8a8495; border-color:rgba(49,69,91,.16); }
.radio-app.theme-light .empty-state strong { color:#3c4753; }
.radio-app.theme-light .card-title { color:#497bb4; }
.radio-app.theme-light .card-title h3 { color:#313a44; }
.radio-app.theme-light .card-title p,.radio-app.theme-light .field-help { color:#808993; }
.radio-app.theme-light .form-grid label,.radio-app.theme-light .editor-card > label { color:#68717b; }
.radio-app.theme-light .program-row,.radio-app.theme-light .schedule-row,.radio-app.theme-light .request-row,.radio-app.theme-light .member-row,.radio-app.theme-light .voice-row { border-color:rgba(49,69,91,.11); background:rgba(248,248,252,.9); }
.radio-app.theme-light .program-icon,.radio-app.theme-light .request-kind,.radio-app.theme-light .member-avatar,.radio-app.theme-light .voice-avatar { color:#4a7db6; background:rgba(76,153,238,.12); }
.radio-app.theme-light .member-avatar.player { color:#267e75; background:rgba(67,193,177,.13); }
.radio-app.theme-light .program-main strong,.radio-app.theme-light .request-main strong,.radio-app.theme-light .member-main strong,.radio-app.theme-light .voice-main strong { color:#343e49; }
...
.radio-app.theme-light .days > span,.radio-app.theme-light .schedule-time span { color:#79828d; }
.radio-app.theme-light .days button { color:#77808b; border-color:rgba(49,69,91,.14); background:rgba(255,255,255,.7); }
.radio-app.theme-light .days button.selected { color:#fff; background:#5291d7; border-color:#6ba5e5; }
...
.radio-app.theme-light .schedule-help p,.radio-app.theme-light .request-guide p,.radio-app.theme-light .voice-admin-note p { color:#68717b; }
.radio-app.theme-light .help-art { color:#4a7db6; background:rgba(76,153,238,.13); }
.radio-app.theme-light .help-line,.radio-app.theme-light .guide-step { color:#5b6570; }
.radio-app.theme-light .schedule-days span { color:#4a72a1; background:rgba(76,153,238,.14); }
.radio-app.theme-light .schedule-program { color:#656e78; }
...
.radio-app.theme-light .kind-tabs button { color:#717a84; border-color:rgba(49,69,91,.13); background:rgba(255,255,255,.65); }
.radio-app.theme-light .kind-tabs button.selected { color:#3e6898; background:rgba(76,153,238,.11); border-color:rgba(76,153,238,.35); }
.radio-app.theme-light .inline-note { color:#747d88; }
.radio-app.theme-light .request-action { color:#5e6975; border-color:rgba(49,69,91,.14); background:rgba(255,255,255,.72); }
.radio-app.theme-light .request-action.preview { color:#446f9f; border-color:rgba(76,153,238,.25); background:rgba(76,153,238,.1); }
.radio-app.theme-light .request-action.download { color:#2d8b5d; border-color:rgba(55,166,107,.22); background:rgba(55,166,107,.09); }
.radio-app.theme-light .request-preview { border-color:rgba(76,153,238,.24); background:linear-gradient(110deg,rgba(236,227,252,.82),rgba(255,255,255,.88)); }
.radio-app.theme-light .request-preview-copy strong { color:#365273; }
.radio-app.theme-light .request-preview-copy span { color:#77808b; }
.radio-app.theme-light .store-checks { color:#747d88; }
.radio-app.theme-light .store-checks > span { color:#68717b; }
.radio-app.theme-light .store-checks label { color:#5e6975; border-color:rgba(49,69,91,.13); background:rgba(255,255,255,.68); }
...
.radio-app.theme-light .member-row select { color:#37424e; background:#fff; border-color:rgba(49,69,91,.15); }
.radio-app.theme-light .voice-main small { color:#717a84; }
.radio-app.theme-light .voice-row .button.ghost { color:#456f9e; border-color:rgba(76,153,238,.25); background:rgba(76,153,238,.08); }
.radio-app.theme-light .player-dock { color:#29333e; background:rgba(255,255,255,.96); border-top-color:rgba(42,59,77,.14); box-shadow:0 -12px 32px rgba(43,64,87,.1); }
.radio-app.theme-light .now-playing strong { color:#303944; }
.radio-app.theme-light .now-playing span,.radio-app.theme-light .progress-line,.radio-app.theme-light .player-tools { color:#767f8a; }
...
.radio-app .brand-mark { background:linear-gradient(135deg,var(--jv-blue),var(--jv-navy)); box-shadow:0 6px 18px rgba(23,61,112,.2); }
.radio-app .button.primary,.radio-app .control-buttons .play-button,.radio-app .days button.selected { background:var(--jv-blue); border-color:var(--jv-blue); box-shadow:0 7px 17px rgba(23,61,112,.2); }
.radio-app .button.primary:hover,.radio-app .control-buttons .play-button:hover { background:var(--jv-navy); }
.radio-app .nav-item.active { box-shadow:inset 3px 0 0 var(--jv-blue); }
.radio-app :is(button, a, input, select, textarea):focus-visible { outline:2px solid var(--jv-blue); outline-offset:2px; }
.radio-app :is(input, select, textarea) { accent-color:var(--jv-blue); }

.radio-app.theme-light {
  color:var(--jv-ink);
  background:radial-gradient(circle at 8% -12%,rgba(58,131,213,.18),transparent 31rem),radial-gradient(circle at 104% 24%,rgba(72,157,128,.12),transparent 27rem),linear-gradient(180deg,#f8fbff,#f3f7fb);
}
.radio-app.theme-light .radio-sidebar { background:rgba(255,255,255,.88); border-right-color:var(--jv-line); box-shadow:10px 0 30px rgba(26,68,113,.04); backdrop-filter:blur(16px); }
.radio-app.theme-light .brand-lockup strong,.radio-app.theme-light .crumb b,.radio-app.theme-light .section-heading h2,.radio-app.theme-light .page-heading h1 { color:var(--jv-navy); }
.radio-app.theme-light .brand-lockup span,.radio-app.theme-light .crumb,.radio-app.theme-light .page-heading p { color:var(--jv-muted); }
.radio-app.theme-light .nav-item { color:#536a82; }
.radio-app.theme-light .nav-item:hover { color:var(--jv-navy); background:#eff6fd; }
.radio-app.theme-light .nav-item.active { color:var(--jv-blue); background:linear-gradient(100deg,#edf6ff,#f8fbff); }
.radio-app.theme-light .topbar { border-bottom-color:var(--jv-line); }
.radio-app.theme-light .station-switcher,.radio-app.theme-light .private-badge,.radio-app.theme-light .cache-badge { border-color:var(--jv-line); background:#f8fbff; }
.radio-app.theme-light .station-switcher,.radio-app.theme-light .station-switcher select,.radio-app.theme-light .station-add,.radio-app.theme-light .theme-toggle { color:var(--jv-blue); }
.radio-app.theme-light .icon-button { color:var(--jv-muted); border-color:var(--jv-line); background:#fff; }
.radio-app.theme-light .icon-button:hover { color:var(--jv-blue); border-color:#a9c8ea; }
.radio-app.theme-light .hero-panel { color:#eff7ff; border-color:rgba(161,203,246,.42); background:linear-gradient(124deg,#173d70 0%,#1e579d 64%,#2f78c9 100%); box-shadow:0 22px 46px rgba(23,61,112,.17); }
.radio-app.theme-light .hero-copy h1,.radio-app.theme-light .hero-copy h1 em { color:#fff; }
.radio-app.theme-light .hero-copy h1 em { color:#bfe1ff; }
.radio-app.theme-light .hero-copy p { color:#d7e9fb; }
.radio-app.theme-light .hero-panel .eyebrow { color:#bfe1ff; }
.radio-app.theme-light .hero-panel .eyebrow-dot { background:#9ed2ff; }
.radio-app.theme-light .hero-disc { color:rgba(219,239,255,.55); border-color:rgba(219,239,255,.24); }
.radio-app.theme-light .hero-panel .button.primary { background:#fff; color:var(--jv-navy); box-shadow:0 8px 20px rgba(10,39,78,.15); }
.radio-app.theme-light .hero-panel .button.primary:hover { background:var(--jv-sky); }
.radio-app.theme-light .hero-panel .button.ghost { color:#fff; border-color:rgba(255,255,255,.5); background:rgba(255,255,255,.08); }
.radio-app.theme-light .hero-panel .button.ghost:hover { background:rgba(255,255,255,.18); }
.radio-app.theme-light .stat-card,.radio-app.theme-light .editor-card,.radio-app.theme-light .list-card { border-color:var(--jv-line); background:#fff; box-shadow:0 12px 28px rgba(23,61,112,.05); }
.radio-app.theme-light .stat-card strong,.radio-app.theme-light .card-title h3,.radio-app.theme-light .track-mini strong,.radio-app.theme-light .track-info strong { color:var(--jv-ink); }
.radio-app.theme-light .stat-card span,.radio-app.theme-light .stat-card small,.radio-app.theme-light .card-title p,.radio-app.theme-light .field-help,.radio-app.theme-light .track-info span { color:var(--jv-muted); }
.radio-app.theme-light .stat-icon.purple,.radio-app.theme-light .program-icon,.radio-app.theme-light .request-kind,.radio-app.theme-light .member-avatar,.radio-app.theme-light .voice-avatar { color:var(--jv-blue); background:var(--jv-sky); }
.radio-app.theme-light .eyebrow,.radio-app.theme-light .text-button,.radio-app.theme-light .card-title,.radio-app.theme-light .mini-play,.radio-app.theme-light .track-info b { color:var(--jv-blue); }
.radio-app.theme-light .genre-card { border-color:#cde0f5; background:linear-gradient(145deg,#deedfb,#f8fbff); color:var(--jv-blue); }
...
.radio-app.theme-light .guide-gradient { background:linear-gradient(135deg,var(--jv-blue),#3a85d2); color:#fff; }

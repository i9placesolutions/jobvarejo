...
const ELEMENT_TYPES = [
  { type: 'image', slot: 'logo', label: 'Logo', icon: '🏪', defaultW: '25%', defaultH: '60%', defaultX: '5%', defaultY: '20%' },
  { type: 'image', slot: 'background_image', label: 'Fundo', icon: '🖼️', defaultW: '100%', defaultH: '100%', defaultX: '0%', defaultY: '0%' },
  { type: 'text', slot: 'company_name', label: 'Nome Empresa', icon: '🏢', defaultW: '60%', defaultH: '30%', defaultX: '30%', defaultY: '20%' },
  { type: 'text', slot: 'slogan', label: 'Slogan', icon: '💬', defaultW: '60%', defaultH: '15%', defaultX: '30%', defaultY: '55%' },
  { type: 'text', slot: 'promo_phrase', label: 'Frase Promo', icon: '📢', defaultW: '80%', defaultH: '15%', defaultX: '10%', defaultY: '78%' },
  { type: 'shape', slot: '', label: 'Forma/Deco', icon: '🔶', defaultW: '100%', defaultH: '20%', defaultX: '0%', defaultY: '80%' },
] as const
...
const MOCK_HEADER: Record<string, string> = {
  logo: '🏪',
  background_image: '',
  company_name: 'MEU SUPERMERCADO',
  slogan: 'O melhor preco da cidade!',
  promo_phrase: 'OFERTAS IMPERDIVEIS',
}
...
const fetchData = async () => {
  ...
  try {
    const data = await $fetch<any>('/api/admin/builder/header-templates', { headers: await getApiAuthHeaders() })
    items.value = (Array.isArray(data) ? data : data?.headerTemplates ?? [])
...
const saveItem = async () => {
  ...
  try {
    ...
    if (editingId.value) {
      await $fetch(`/api/admin/builder/header-templates/${editingId.value}`, { method: 'PUT', body: payload, headers: await getApiAuthHeaders() })
    } else {
      await $fetch('/api/admin/builder/header-templates', { method: 'POST', body: payload, headers: await getApiAuthHeaders() })
    }
    mode.value = 'list'
    await fetchData()
...
const deleteItem = async (id: string) => {
  ...
  try {
    await $fetch(`/api/admin/builder/header-templates/${id}`, { method: 'DELETE', headers: await getApiAuthHeaders() })
    await fetchData()
      ...
      <div class="p-6 max-w-6xl mx-auto">
        <div class="flex items-center justify-between mb-6">
          <div>
            ...
            <p class="text-sm text-gray-500 mt-1">Templates de cabecalho do encarte. Crie headers visuais, o cliente escolhe.</p>
          </div>
            ...
            >
              ...
              >
                ...
                <div :style="elementPreviewStyle(el)">
                  ...
                  <div v-if="el.type === 'image'" :style="{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', background: el.slot === 'background_image' ? 'linear-gradient(135deg, #667eea, #764ba2)' : 'rgba(255,255,255,0.15)', borderRadius: el.borderRadius || '4px', fontSize: '20px' }">{{ el.slot === 'logo' ? '🏪' : '🖼️' }}</div>

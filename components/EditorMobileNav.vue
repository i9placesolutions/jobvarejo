...
const buttons = [
  { id: 'tools', label: 'Adicionar', icon: MousePointer2 },
  { id: 'layers', label: 'Camadas', icon: Layers },
  { id: 'properties', label: 'Editar', icon: SlidersHorizontal },
...
<template>
  <div class="editor-mobile-nav fixed bottom-0 left-0 right-0 z-[9990] bg-[#18181b]/95 backdrop-blur-xl border-t border-white/10">
    <nav aria-label="Ferramentas do editor" class="grid grid-cols-5 gap-1 px-2 pt-1.5 pb-[calc(0.55rem+env(safe-area-inset-bottom,0px))]">
      <button
        ...
        :aria-expanded="activePanel === btn.id"
        @click="toggle(btn.id)"
      >
        <component :is="btn.icon" :size="20" :stroke-width="1.8" />
        <span class="text-[11px] font-medium leading-tight">{{ btn.label }}</span>
      </button>
    </nav>
  </div>
</template>

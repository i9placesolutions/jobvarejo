...
const emit = defineEmits<{
  close: []
  add: [items: Array<{ name: string; price: number | null }>]
}>()
...
const parseProductList = (raw: string): ParsedProduct[] => {
  ...
  for (const line of lines) {
    ...
    // Tentar casar: nome <separador> [R$] preco
          ...
          <div class="px-5 py-4 space-y-3">
            ...
            <textarea
              ...
              class="w-full bg-[#09090b]/50 text-[12px] text-white placeholder-zinc-600 outline-none
                border border-white/5 focus:border-violet-500/50 rounded-lg px-3 py-2.5 transition-colors
            ...
            <Transition
              ...
              leave-to-class="opacity-0"
            >
              <p v-if="addedCount !== null" class="text-xs text-violet-400 font-medium">

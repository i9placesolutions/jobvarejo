...
import { loadCartazistaFonts as loadArtFonts } from '~/utils/cartazista/fonts'
  ...
  try {
    ...
    for (const layer of doc.layers) {
      ...
      if (layer.kind === 'text' && layer.textArc) {
        ...
        while (
          (text.height > layer.height + .01 || text.width * text.scaleX > layer.width + .01) &&
          text.fontSize > 6
        ) {
          text.set({styles:cartazistaPriceStyles(layer,text.fontSize-1),fontSize: text.fontSize - 1, width: layer.width / (layer.fontScaleX || 1)})
          text.initDimensions()
        }
        if (text.height > layer.height + .01 || text.width * text.scaleX > layer.width + .01)
          layoutError = true
        obj = text

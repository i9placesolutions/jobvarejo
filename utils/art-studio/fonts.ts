  ...
  await Promise.all(
    requests.map(({ family, weight }) => {
      ...
      if (!loaded.has(key)) {
        ...
        const special: Record<string,string> = {'Knewave':'Knewave-Regular.ttf','Montserrat':'Montserrat[wght].ttf','Patua One':'PatuaOne-Regular.ttf','Consumidor Referencia':'ConsumidorReferencia-Regular.ttf','Russo One':'RussoOne-Regular.ttf','Roboto Slab':'RobotoSlab[wght].ttf',Audiowide:'Audiowide-Regular.ttf','Bebas Neue':'BebasNeue-Regular.ttf',Caveat:'Caveat[wght].ttf'}

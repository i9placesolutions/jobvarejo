...
import customRecipes from '../../shared/video-studio/custom-flyer-recipes.json'
describe('Direção temática do catálogo',()=>{
 ...
 it('preserva famílias visuais e usa acentos curtos sem explosões repetidas',()=>{
  ...
  for(const r of Object.values(FLYER_RECIPES)){
   ...
   const custom=customRecipes.find(item=>item.sourceProject===r.sourceProject)
   expect(r.motion.accentSound).toBe(custom?.motion.accentSound||'retail-pop-v1')
   expect(SOUND_EFFECTS.some(s=>s.id===r.motion.accentSound)).toBe(true)

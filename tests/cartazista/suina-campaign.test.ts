...
describe('Famílias Quarta Suína',()=>{
 ...
 const profile={companyName:'Loja B',whatsapp:'11999999999',address:'Endereço B',instagram:'@lojaB'};const b=hydrateCartazistaBusiness(r.composition,profile,'/logo-b.png');expect(b.layers.find(l=>l.binding==='phone')?.text).toBe(profile.whatsapp);expect(b.layers.find(l=>l.binding==='address')?.text).toBe(profile.address);
 const empty=hydrateCartazistaBusiness(b,{companyName:'',whatsapp:'',address:'',instagram:''},'');expect(empty.layers.filter(l=>l.binding&&l.visible)).toHaveLength(0);expect(r.composition.layers.find(l=>l.binding==='phone')?.text).toBe('');
 });
});

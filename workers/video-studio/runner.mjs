...
async function provider(job,label,payload,type){
 const state=job.provider_state||{};let record=state[label]
 if(record?.asset)return record.asset
 if(record?.sending&&!record.taskId)throw Error('O provedor pode ter recebido esta solicitação. O suporte deve conferir antes de cobrar uma nova geração.')
 const key=cfg('MUSICGPT_API_KEY');if(!key)throw Error('A locução não está configurada no servidor.')
 ...
 if(!record?.taskId){state[label]={sending:true};await checkpoint(job,state,5);const response=await fetch(endpoint,{method:'POST',headers:{Authorization:key,'Content-Type':'application/json'},body:JSON.stringify(payload),signal:AbortSignal.timeout(45000)});if(!response.ok)throw Error('O MusicGPT não aceitou a geração. Confira a configuração e o saldo.');const data=await response.json();const taskId=data.task_id||data.taskId||data.id;if(!taskId)throw Error('O MusicGPT não retornou o identificador. Consulte o suporte antes de tentar novamente.');record=state[label]={taskId,sending:false};await checkpoint(job,state,10)}
 const poll=new URL(endpoint);poll.pathname=poll.pathname.replace(/\/[^/]+$/,'/byId');poll.searchParams.set('task_id',record.taskId);poll.searchParams.set('conversionType',type)
 for(let n=0;n<150;n++){if(stopping)throw Error('Serviço em reinicialização.');const response=await fetch(poll,{headers:{Authorization:key},signal:AbortSignal.timeout(30000)});if(!response.ok){await sleep(8000);continue}const data=await response.json(),c=data.conversion||data;const url=[c.audio_url,c.audioUrl,c.result_url,c.resultUrl,c.output?.url,c.conversion_path_1,c.conversion_path,c.conversion_path_wav_1,c.conversion_path_wav].find(v=>typeof v==='string'&&v.startsWith('https://'));if(url)return {url,state,label};if(['failed','error'].includes(String(c.status||c.state).toLowerCase()))throw Error('O MusicGPT informou falha nesta geração.');await sleep(8000)}throw Error('O áudio ainda não ficou pronto. A solicitação está registrada; consulte o suporte para recuperar sem repetir a cobrança.')
}
...
async function generate(job,dir){
 ...
 if(job.kind==='voice'){
  ...
  const sample=voice.sampleKey?await getSignedUrl(s3,new GetObjectCommand({Bucket:bucket,Key:voice.sampleKey}),{expiresIn:3600}):null
  for(let i=0;i<scripts.length;i++){const script=scripts[i],identity=hash({text:script.text,voice});const cached=(await pool.query("SELECT id,duration FROM public.video_studio_assets WHERE user_id=$1 AND kind='voice' AND metadata->>'fingerprint'=$2 ORDER BY created_at DESC LIMIT 1",[job.user_id,identity])).rows[0];if(cached){results[script.id]={assetId:cached.id,duration:cached.duration,text:script.text};continue}
   const out=await provider(job,script.id,{text:script.text,gender:voice.gender,...(sample?{sample_audio_url:sample}:{voice_id:voice.voiceId})},'TEXT_TO_SPEECH');let asset
   if(out.id)asset=out;else{const bytes=await providerBytes(out.url),file=join(dir,'voice.audio');await writeFile(file,bytes);const info=await probe(file);const duration=Number(info.format?.duration);if(!info.streams?.some(s=>s.codec_type==='audio')||!Number.isFinite(duration)||duration>120)throw Error('O provedor retornou um áudio inválido.');asset=await store(job,'voice',script.text.slice(0,100),bytes,'audio/mpeg','mp3',duration,{fingerprint:identity});out.state[script.id].asset=asset;await checkpoint(job,out.state,Math.round((i+1)/scripts.length*90))}results[script.id]={assetId:asset.id,duration:asset.duration,text:script.text}
  }
  const durations=Object.fromEntries(Object.entries(results).map(([k,v])=>[k,v.duration]));const scenes=buildVideoTimeline(doc,durations);return {audioIdentity:videoAudioIdentity(doc),clips:results,scenes}
 }
 ...
 const scenes=buildVideoTimeline(doc,doc.voice.enabled?Object.fromEntries(Object.entries(voice.clips).map(([id,c])=>[id,c.duration])):undefined)
 for(const s of scenes)if(doc.voice.enabled)s.audio=(await assetFile(voice.clips[s.id].assetId,job.user_id,dir)).name
 let music

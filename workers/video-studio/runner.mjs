...
async function generate(job,dir){
 ...
 if(job.kind==='voice'){
  ...
  const identity=hash({mode:'full-v1',speechVersion:2,texts:scripts.map(s=>s.text),voice})
  ...
  // Referências antigas já tinham chave própria, mas não registravam sampleVersion.
  // Só reutilizar áudio criado depois da preparação atual da mesma referência.
  if(!asset&&voice.sampleVersion&&Number.isFinite(Date.parse(voice.sampleVersion))){
   const {sampleVersion,...legacyVoice}=voice
   const legacyIdentities=[hash({mode:'full-v1',speechVersion:2,texts:scripts.map(s=>s.text),voice:legacyVoice}),hash({mode:'full-v1',texts:scripts.map(s=>s.text),voice:legacyVoice})]
   asset=(await pool.query("SELECT id,duration,metadata FROM video_studio_assets WHERE user_id=$1 AND kind='voice' AND metadata->>'fingerprint'=ANY($2::text[]) AND created_at >= $3::timestamptz ORDER BY created_at DESC LIMIT 1",[job.user_id,legacyIdentities,sampleVersion])).rows[0]
   if(asset){asset.metadata={...asset.metadata,fingerprint:identity,sampleVersion,legacyFingerprint:asset.metadata.fingerprint};await pool.query('UPDATE video_studio_assets SET metadata=$1::jsonb WHERE id=$2 AND user_id=$3',[JSON.stringify(asset.metadata),asset.id,job.user_id])}
  }
  if(!asset){
   const out=await provider(job,'full-voice',{text:fullVoiceText(scripts),gender:voice.gender,...(sample?{sample_audio_url:sample}:{voice_id:voice.voiceId})},'TEXT_TO_SPEECH')
   if(out.id){asset=(await pool.query('SELECT id,duration,metadata FROM video_studio_assets WHERE id=$1 AND user_id=$2',[out.id,job.user_id])).rows[0]}
   else{const bytes=await providerBytes(out.url),file=join(dir,'full-voice.audio');await writeFile(file,bytes);const info=await probe(file);const duration=Number(info.format?.duration);if(!info.streams?.some(s=>s.codec_type==='audio')||!Number.isFinite(duration)||duration<=0||duration>180)throw Error('O serviço retornou um áudio inválido.');const metadata={fingerprint:identity,mode:'full-v1',speechVersion:2,voiceProfileId:voice.profileId||null,sampleVersion:voice.sampleVersion||null,usage:voiceUsage(fullVoiceText(scripts))};asset={...await store(job,'voice','Locução completa — '+doc.title,bytes,'audio/mpeg','mp3',duration,metadata),metadata};out.state['full-voice'].asset={id:asset.id,duration};await checkpoint(job,out.state,95)}
  }
  ...
  if(asset.metadata?.alignment?.version!=='words-v1'){
  ...
  return {audioIdentity:videoAudioIdentity(doc),fullVoice,clips:{},scenes:fullVoiceTimeline(doc,fullVoice)}
 }

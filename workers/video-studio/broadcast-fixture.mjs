...
for(const format of ['vertical','horizontal']){const props={document:doc,media,scenes,format,music:'music.mp3',impact:'impact.mp3',whoosh:'whoosh.mp3'}
 if(process.argv.includes('--stills')){for(const [name,frame] of [['opening',30],['offer',100],['ending',scenes.at(-1).from+40]]){await renderVideo(props,dir,join(dir,format+'-'+name+'.png'),()=>{},{frame,scale:.7});console.log(format,name)}}
 else{await renderVideo(props,dir,join(dir,format+'.mp4'),p=>{});console.log(format,'render complete')}
}

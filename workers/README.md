...
As dependências, Chromium headless e pesos são instalados em camadas separadas. O modelo é uma cópia dos pesos oficiais `BiRefNet-general-epoch_244.onnx` do rembg no Wasabi do projeto, verificada pelo MD5 oficial `7a35a0141cbbc80de11d9c9a28f52697`. O cache BuildKit mantém downloads parciais; `curl --continue-at` retoma e o checksum é obrigatório antes de copiar para a imagem. A inicialização testa Chromium, inferência em produto real e busca pública; `/api/health` expõe o resultado em `imageRuntime`.

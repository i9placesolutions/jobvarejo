...
export default defineEventHandler(async event=>{
 ...
 return pgTx(async client=>{
  ...
  if(input.kind==='render'&&doc.voice.enabled){const r=await client.query("SELECT result FROM public.video_studio_jobs WHERE user_id=$1 AND project_id=$2 AND kind='voice' AND status='ready' AND payload->>'audioHash'=$3 ORDER BY created_at DESC LIMIT 1",[u.id,p.id,audioHash]);voiceResult=r.rows[0]?.result;if(!voiceResult)throw createError({statusCode:422,statusMessage:'Gere e ouça a locução desta versão antes de exportar.'})}

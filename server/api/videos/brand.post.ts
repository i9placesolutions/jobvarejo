...
import { videoUser,putVideoAsset,videoBucket } from '../../utils/video-studio/service'
...
export default defineEventHandler(async event=>{const u=await videoUser(event,30);const profile=await pgOneOrNull<any>('SELECT business_profile FROM public.profiles WHERE id=$1',[u.id]);const b=normalizeBusinessProfile(profile?.business_profile);let logo='';let warning=''

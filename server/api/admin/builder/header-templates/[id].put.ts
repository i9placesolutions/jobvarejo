...
export default defineEventHandler(async (event) => {
  ...
  enforceRateLimit(event, `admin-header-templates-update:${user.id}`, 60, 60_000)
  ...
  const item = await pgOneOrNull(
    `UPDATE public.builder_header_templates SET ${setClauses.join(', ')} WHERE id = $${pi}::uuid RETURNING *`,
    values
  )

  if (!item) throw createError({ statusCode: 404, statusMessage: 'Not found' })
  return { headerTemplate: item }
})

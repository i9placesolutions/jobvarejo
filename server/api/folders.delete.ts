...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    if (!row?.id) throw createError({ statusCode: 404, statusMessage: 'Folder not found' })

    return { success: true }
  } catch (error: any) {
    if (error?.statusCode) throw error
    throw createError({ statusCode: 500, statusMessage: error?.message || 'Failed to delete folder' })
  }
})

...
export const videoDocumentSchema=z.object({
 ...
 offers:z.array(z.object({copies:z.union([z.literal(1),z.literal(2),z.literal(3)]).optional(),imageAspectRatio:z.number().min(.05).max(20).optional(),id:z.string().uuid(),name:text(120),price:text(20),unit:text(30),image:asset,condition:text(140)})).max(6).refine(v=>new Set(v.map(o=>o.id)).size===v.length),

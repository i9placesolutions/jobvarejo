...
export async function resolveVideoVoice(id:string,userId:string){
 ...
 if(/^[a-f0-9-]{36}$/i.test(id)){const row=await pgOneOrNull<any>("SELECT id,gender,metadata FROM public.radio_voice_profiles WHERE id=$1 AND user_id=$2 AND station_id IS NULL AND status='active' AND consent_status='confirmed'",[id,userId]);const key=String(row?.metadata?.cloneSampleKey||'');if(key.startsWith(`radio-indoor/voices/${userId}/`)&&!key.includes('..'))return {sampleKey:key,gender:row.gender,profileId:id,sampleVersion:String(row.metadata.clonePreparedAt||'')}}

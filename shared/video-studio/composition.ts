...
function Scene({scene,props}:{scene:VideoScene;props:VideoRenderProps}){
 ...
 const footer=div({position:'absolute',bottom:portrait?65:65,left:portrait?75:offer?1050:85,right:portrait?75:85,textAlign:'center',borderTop:`1px solid ${t.accent}55`,paddingTop:22,display:'flex',flexDirection:'column',gap:10},offer?div({height:portrait?150:95,display:'flex',justifyContent:'center'},logo):null,offer&&validity?div({display:'flex',alignItems:'center',justifyContent:'center',gap:10,fontSize:fit(validity,small,80),fontWeight:700,color:'var(--video-validity-color, var(--video-text-color, white))'},h(SocialIcon,{kind:'calendar',size:28}),validity):null,div({fontSize:fit([d.brand.address,d.brand.whatsapp,d.brand.instagram].filter(Boolean).join(' • '),small-3,100),color:'var(--video-contact-color, #e2dce2)',lineHeight:1.3},[d.brand.address,d.brand.whatsapp,d.brand.instagram].filter(Boolean).join(' • ')))
 ...
 if(intro||outro)content=div({position:'absolute',inset:portrait?'28% 8% 26%':'27% 12% 24%',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:portrait?55:35,textAlign:'center',transform:`translateY(${(1-enter)*70}px) scale(${.92+.08*enter})`},div({border:`1px solid ${t.accent}77`,borderRadius:60,padding:'12px 32px',fontSize:portrait?29:27,letterSpacing:4,color:'#fff'},outro?'APROVEITE AS OFERTAS':'PREPARE-SE PARA ECONOMIZAR'),campaign,outro?div({fontSize:fit(d.brand.name,portrait?75:70,35),fontWeight:800,lineHeight:1.05},d.brand.name):div({fontSize:portrait?38:35,color:'#eee2df'},'Confira os destaques da nossa loja'))
  ...
  const name=div({color:'var(--video-name-color, var(--video-text-color, white))',fontFamily:'VideoCondensed, sans-serif',fontSize:fit(offer.name,portrait?58:60,48),fontWeight:800,lineHeight:1.05,textTransform:'uppercase',overflowWrap:'break-word'},offer.name)
  const price=div({color:'var(--video-price-color, white)',position:'relative',width:portrait?'100%':undefined,boxSizing:'border-box',justifyContent:'center',display:'inline-flex',alignItems:'center',gap:18,background:'#e3333e',border:'3px solid #ff7279',borderRadius:32,padding:portrait?'20px 42px':'22px 38px',boxShadow:'0 20px 70px #0005',overflow:'hidden'},div({fontSize:38,fontWeight:700,alignSelf:'flex-start',paddingTop:16},'R$'),div({fontFamily:'VideoCondensed, sans-serif',fontWeight:800,fontSize:fit(integer,portrait?205:154,5),lineHeight:.9},integer),div({display:'flex',flexDirection:'column',gap:8},div({fontFamily:'VideoCondensed, sans-serif',fontSize:portrait?94:74,fontWeight:800,lineHeight:1},','+cents),div({color:'var(--video-unit-color, white)',fontSize:fit(offer.unit,28,12),fontWeight:700,maxWidth:210},offer.unit)),d.effects.includes('glow')?div({position:'absolute',inset:0,background:'linear-gradient(110deg,transparent 30%,#ffffff40 48%,transparent 65%)',transform:`translateX(${((f%120)/120)*300-150}%)`}):null)
  const productWidth=portrait?reels.product[2]:100,productHeight=portrait?reels.product[3]:100
  const product=div({position:'relative',height:portrait?productHeight:380,width:'100%',display:'flex',justifyContent:'center',alignItems:'center',transform:`translate(${shake}px,${bounce}px) scale(${zoom*pulse})`},div({position:'absolute',width:'80%',height:'80%',background:`radial-gradient(ellipse,${t.accent}22,transparent 65%)`,borderRadius:'50%'}),...productLayers([0,0,productWidth,productHeight],portrait,d.duplicateProducts!==false,offer.imageAspectRatio||1,offer.copies).map(layer=>h('div',{key:layer.motionIndex,style:{position:'absolute',left:layer.box[0]/productWidth*100+'%',top:layer.box[1]/productHeight*100+'%',width:layer.box[2]/productWidth*100+'%',height:layer.box[3]/productHeight*100+'%',rotate:layer.rotation+'deg'}},offer.image&&media(offer.image)?h(Img,{src:media(offer.image),style:{width:'100%',height:'100%',objectFit:'contain',filter:'drop-shadow(0 28px 28px #0008)'}}):div({fontSize:portrait?150:170,opacity:.3},'✦'))))
  const slot=(area:readonly [number,number,number,number],child:React.ReactNode)=>div({position:'absolute',left:area[0],top:area[1],width:area[2],height:area[3],display:'flex',alignItems:'center',justifyContent:'center',textAlign:'center'},child)
  content=portrait?h(AbsoluteFill,null,slot(reels.seal,campaign),slot(reels.name,name),slot(reels.product,product),slot(reels.price,props.label?h(VideoPriceLabel,{label:props.label,price:offer.price,unit:offer.unit,colors:d.appearance}):price),slot(reels.condition,div({color:'var(--video-condition-color, var(--video-text-color, white))',fontSize:fit(offer.condition,30,75),lineHeight:1.2},offer.condition))):

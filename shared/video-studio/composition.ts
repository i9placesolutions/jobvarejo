...
function Scene({scene,props}:{scene:VideoScene;props:VideoRenderProps}){
 ...
 const header=div({position:'absolute',top:portrait?140:58,left:portrait?70:85,right:portrait?70:85,display:'flex',alignItems:'center',justifyContent:'space-between',gap:35},div({display:'flex',alignItems:'center',gap:22},logo,div({fontSize:fit(d.brand.name,portrait?43:37,30),fontWeight:800,maxWidth:portrait?(logo?450:710):1050,lineHeight:1.1,overflowWrap:'anywhere'},d.brand.name||'Sua empresa')),div({color:t.accent,fontSize:portrait?24:23,letterSpacing:3,fontWeight:700},'OFERTAS'))
 ...
 const footer=div({position:'absolute',bottom:portrait?185:65,left:portrait?75:85,right:portrait?75:85,textAlign:'center',borderTop:`1px solid ${t.accent}55`,paddingTop:22,display:'flex',flexDirection:'column',gap:10},div({fontSize:fit(d.validity,small,80),fontWeight:700},d.validity),div({fontSize:fit([d.brand.address,d.brand.whatsapp,d.brand.instagram].filter(Boolean).join(' • '),small-3,100),color:'#e2dce2',lineHeight:1.3},[d.brand.address,d.brand.whatsapp,d.brand.instagram].filter(Boolean).join(' • ')))
 ...
 if(intro||outro)content=div({position:'absolute',inset:portrait?'28% 8% 26%':'27% 12% 24%',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:portrait?55:35,textAlign:'center',transform:`translateY(${(1-enter)*70}px) scale(${.92+.08*enter})`},div({border:`1px solid ${t.accent}77`,borderRadius:60,padding:'12px 32px',fontSize:portrait?29:27,letterSpacing:4,color:'#fff'},outro?'APROVEITE AS OFERTAS':'PREPARE-SE PARA ECONOMIZAR'),campaign,outro?div({fontSize:fit(d.brand.name,portrait?75:70,35),fontWeight:800,lineHeight:1.05},d.brand.name):div({fontSize:portrait?38:35,color:'#eee2df'},'Confira os destaques da nossa loja'))
  ...
  const name=div({fontFamily:'VideoCondensed, sans-serif',fontSize:fit(offer.name,portrait?85:92,48),fontWeight:800,lineHeight:1.05,textTransform:'uppercase',overflowWrap:'break-word'},offer.name)
  const price=div({position:'relative',display:'inline-flex',alignItems:'center',gap:18,background:'#e3333e',border:'3px solid #ff7279',borderRadius:32,padding:portrait?'20px 42px':'22px 38px',boxShadow:'0 20px 70px #0005',overflow:'hidden'},div({fontSize:38,fontWeight:700,alignSelf:'flex-start',paddingTop:16},'R$'),div({fontFamily:'VideoCondensed, sans-serif',fontWeight:800,fontSize:fit(integer,portrait?158:154,5),lineHeight:.9},integer),div({display:'flex',flexDirection:'column',gap:8},div({fontFamily:'VideoCondensed, sans-serif',fontSize:74,fontWeight:800,lineHeight:1},','+cents),div({fontSize:fit(offer.unit,28,12),fontWeight:700,maxWidth:210},offer.unit)),d.effects.includes('glow')?div({position:'absolute',inset:0,background:'linear-gradient(110deg,transparent 30%,#ffffff40 48%,transparent 65%)',transform:`translateX(${((f%120)/120)*300-150}%)`}):null)
  const product=div({position:'relative',height:portrait?550:610,width:'100%',display:'flex',justifyContent:'center',alignItems:'center',transform:`translate(${shake}px,${bounce}px) scale(${zoom*pulse})`},div({position:'absolute',width:'80%',height:'80%',background:`radial-gradient(ellipse,${t.accent}22,transparent 65%)`,borderRadius:'50%'}),offer.image&&media(offer.image)?h(Img,{src:media(offer.image),style:{position:'relative',width:'100%',height:'100%',objectFit:'contain',filter:'drop-shadow(0 28px 28px #0008)'}}):div({fontSize:portrait?150:170,opacity:.3},'✦'))
  content=portrait?div({position:'absolute',top:320,left:85,right:85,bottom:340,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'space-around',gap:22,textAlign:'center'},campaign,name,product,price,div({fontSize:fit(offer.condition,30,75),lineHeight:1.2},offer.condition)):
   div({position:'absolute',top:225,left:100,right:100,bottom:200,display:'grid',gridTemplateColumns:'1fr 1fr',alignItems:'center',gap:90},product,div({display:'flex',flexDirection:'column',gap:34,alignItems:'flex-start'},pro?campaign:div({fontSize:26,color:t.accent,fontWeight:700,letterSpacing:2},d.campaign),name,price,div({fontSize:fit(offer.condition,28,75)},offer.condition)))
 }
 const transition=d.transition==='slide'?`translateX(${(1-enter)*width*.16}px)`:'none'
 return h(AbsoluteFill,{style:{opacity:Math.min(enter,exit),transform:transition,color:'white'}},header,content,footer,d.transition==='light'&&f<15?div({position:'absolute',inset:0,background:`linear-gradient(110deg,transparent,${t.accent}66,transparent)`,transform:`translateX(${(f/15)*220-110}%)`}):null,d.transition==='smoke'&&f<22?div({position:'absolute',inset:0,background:'radial-gradient(ellipse,#c9bbaaaa,transparent 75%)',opacity:1-f/22,filter:'blur(35px)'}):null)
}
export function VideoComposition(props:VideoRenderProps){return flyerRecipe(props.document.theme)?h(FlyerComposition,props):props.document.theme==='impact'?h(props.document.layoutVersion===2?ShowcaseComposition:BroadcastComposition,props):h(ClassicComposition,props)}
function ClassicComposition(props:VideoRenderProps){
 ...
 const musicVolume=(frame:number)=>{let duck=1;for(const s of props.scenes){if(!s.audio)continue;const start=s.from,end=s.from+(s.speechFrames||s.frames);const envelope=frame<start?Math.max(0,1-(start-frame)/10):frame<=end?1:Math.max(0,1-(frame-end)/15);duck=Math.min(duck,1-.72*envelope)}return d.audio.musicVolume*duck*Math.max(0,Math.min(1,frame/20,(durationInFrames-frame)/25))}
 return h(AbsoluteFill,{style:{background:`radial-gradient(ellipse at 50% 10%,${theme.accent}22,transparent 55%),linear-gradient(145deg,${theme.base},#080b12)`,fontFamily:'VideoBarlow, sans-serif',overflow:'hidden'}},
  ...
  h(Atmosphere,{doc:d}),
  ...props.scenes.map(s=>h(Sequence,{key:s.id,from:s.from,durationInFrames:s.frames},h(Scene,{scene:s,props}),s.audio&&d.voice.enabled?h(Audio,{src:s.audio,volume:d.audio.voiceVolume}):null,d.audio.sounds&&props.whoosh?h(Audio,{src:props.whoosh,volume:d.audio.effectsVolume*.55}):null,d.audio.sounds&&props.impact&&s.id!=='outro'?h(Sequence,{from:8},h(Audio,{src:props.impact,volume:d.audio.effectsVolume})):null)),

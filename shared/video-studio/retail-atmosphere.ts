...
export function RetailAtmosphere({props}:{props:VideoRenderProps}){
 const f=useCurrentFrame(),{width:w,height:ht}=useVideoConfig(),r=flyerRecipe(props.document.theme),seed=r?.seed||31,accent=r?.accent||'#d8ef85',fx=motionSettings(props.document.motion).atmosphere,s=props.document.intensity,fast=props.fastPreview===true,items:React.ReactNode[]=[]
 ...
 if(fx.includes('smoke-plumes'))for(let i=0;i<(fast?4:10);i++){
  const life=(f*.005+i/10)%1,size=w*(.28+life*.55),x=(i%2?w*.86:-w*.2)+noise2D('smoke',f*.008,i)*w*.1
  items.push(div({position:'absolute',left:x-size*.35,top:ht*(1.15-life*1.5),width:size,height:size*.8,borderRadius:'45% 55% 70% 30%',rotate:`${i*53+f*.4}deg`,background:`radial-gradient(ellipse,${accent}44,#8d92a222 38%,transparent 70%)`,opacity:Math.sin(life*Math.PI)*.65*s,filter:fast?'blur(6px)':'blur(18px)'}))
 }

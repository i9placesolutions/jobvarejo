...
export const VIDEO_THEMES = [
  ...Object.values(FLYER_RECIPES).map(r=>({id:r.id,name:r.name,description:r.layoutName||'Composição própria do encarte.',accent:r.accent,base:r.base,title:r.campaign})),
...
export interface VideoOffer { copies?: 1|2|3; imageAspectRatio?: number; id: string; name: string; price: string; unit: string; image: string; condition: string }
...
export interface VideoDocument {
  ...
  formats: VideoFormat[]; duration: 15 | 20 | 30
...
export interface VideoScene { id: string; from: number; frames: number; audio?: string; speechFrames?: number }
import type { VideoLabel } from './labels'
export interface VideoRenderProps extends Record<string, unknown> { editor?: VideoEditorState; document: VideoDocument; scenes: VideoScene[]; media: Record<string, string>; format: VideoFormat; music?: string; impact?: string; whoosh?: string; audioBase?: string; fontBase?: string; templateBase?: string; label?: VideoLabel }
...
export function buildVideoTimeline(doc: VideoDocument, durations?: Record<string, number>): VideoScene[] {
  ...
  const lengths = ids.map((id,i)=>Math.max(minimums[i]!, doc.voice.enabled ? (durations ? (durations[id] ?? NaN) : estimateSpeechSeconds(doc.scripts.find(s=>s.id===id)?.text||'')) + .25 : minimums[i]!))
  if (lengths.some(n=>!Number.isFinite(n))) throw new Error('A locução de uma das cenas ainda não está pronta.')
  const budget = doc.duration * VIDEO_FPS - 2 // margem para a duração do contêiner AAC/MP4
  const frames = lengths.map(n=>Math.ceil(n*VIDEO_FPS))
  if (frames.reduce((a,b)=>a+b,0)>budget) throw new Error(`O conteúdo ultrapassa ${doc.duration} segundos. Encurte a locução ou use menos produtos.`)
  ...
  let from=0
  return ids.map((id,i)=>{const result={id,from,frames:frames[i]!,speechFrames:durations?.[id] ? Math.ceil(durations[id]!*VIDEO_FPS) : undefined};from+=frames[i]!;return result})
}

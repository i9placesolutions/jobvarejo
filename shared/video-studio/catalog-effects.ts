...
export function CatalogTransition({props}:{props:VideoRenderProps}) {
  ...
  if(mode==='smoke')for(let i=0;i<3;i++)items.push(div({...abs(w*(i*.4-.3)+local*w*.02,ht*(i%2?.5:-.1),w,ht*.8),borderRadius:'50%',background:'radial-gradient(ellipse,#c2ddb7,#668a51aa 40%,transparent 70%)',filter:'blur(25px)',opacity:t.cover}))

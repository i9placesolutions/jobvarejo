...
import { type VideoDocument, displayPrice } from './model'
import type { VideoLabel } from './labels'
import { priceLayout } from './price-layout'
export function VideoPriceLabel({label,price,unit,colors}:{label:VideoLabel;price:string;unit:string;colors?:VideoDocument['appearance']}){
 ...
 return h('svg',{viewBox:`${-label.width/2-5} ${-label.height/2-5} ${label.width+10} ${label.height+10}`,style:{width:'100%',height:'100%',overflow:'visible'}},...label.nodes.map((n,i)=>{
  ...
  if(n.type==='rect')child=h('rect',{...common,x,y,width:n.width,height:n.height,rx:n.rx})
   ...
   const override=/unit_text/.test(n.name)?colors?.unitColor:/currency|symbol/.test(n.name)||text.trim()==='R$'?colors?.currencyColor:/price_(integer|decimal|value)_text/.test(n.name)?colors?.priceColor:undefined
   child=vectorText(text,{...common,...(override?{fill:override,stroke:n.stroke?override:n.stroke}:{}),x:0,y:y+n.height*.79,size:n.fontSize,width:shrink<1?n.width:undefined,anchor,condensed:font==='VideoCondensed'})
  }
  return h('g',{key:i,transform:`translate(${n.x} ${n.y}) rotate(${n.angle}) scale(${n.sx} ${n.sy})`},child)
 }),run?h('g',null,
 vectorText(integer,{x:run.x,y:run.baseline,size:run.size,width:run.wholeWidth,fill:colors?.priceColor||run.fill,stroke:colors?.priceColor||run.fill,strokeWidth:run.size*.013}),
 vectorText(','+cents,{x:run.decimalX,y:run.baseline-run.size*.32,size:run.size*.58,width:run.decimalWidth,fill:colors?.priceColor||run.fill,stroke:colors?.priceColor||run.fill,strokeWidth:run.size*.013}),
 run.unit?vectorText(unit,{x:run.decimalX+run.decimalWidth/2,y:run.baseline,size:run.size*.23,anchor:'middle',fill:colors?.unitColor||run.fill}):null):null)
}

...
try {
 ...
 await writeFile(dir+'/audio-logo-source.json',JSON.stringify({project:p,voice},null,2));
 ...
 if(process.argv.includes('--prepare')) {
   for(const [name,id] of [['audio-logo-music.mp3',p.document.audio.music],...Object.entries(voice.result.clips).map(([id,c])=>['audio-logo-'+id+'.mp3',c.assetId])]) {
 ...
 if(process.argv.includes('--render')) {
   const d=p.document; d.audio.musicVolume=.65;d.audio.effectsVolume=.8;d.audio.sounds=true;
   ...
   await writeFile(dir+'/audio-logo-render-job.json',JSON.stringify(render,null,2));
 ...
 if(process.argv.includes('--download')) {
   ...
   if(job?.status==='ready'){
     for(const {format,assetId} of job.result.outputs){
       ...
       const media=await fetch(r.headers.get('location'));if(!media.ok)throw Error('Download falhou');
       await writeFile(dir+'/audio-logo-'+format+'.mp4',Buffer.from(await media.arrayBuffer()));
     }
     await writeFile(dir+'/audio-logo-result.json',JSON.stringify(job,null,2));
   }
 }

...
const dir=resolve('output/video-all-models/'+(process.env.VIDEO_MOTION_DIR||'motion-review-v12'));await mkdir(dir,{recursive:true})
...
const recipes=Object.values(FLYER_RECIPES),results=[],requested=process.env.VIDEO_MOTION_MODELS?.split(','),selected=requested?recipes.filter(r=>requested.includes(r.id)):recipes.slice(0,8)
...
for(const r of [...selected,...families.filter(r=>!selected.includes(r))]){
 ...
 for(const format of ['vertical','horizontal']){
  ...
  if(selected.includes(r)){
   const file=r.id+'-'+format+'-entry.mp4';await renderVideo(props,dir,dir+'/'+file,()=>{},{frameRange:[scenes[1].from-6,scenes[1].from+36]});results.push({id:r.id,name:r.name,format,kind:'motion',file,transition:r.transition,product:r.motion.product,price:r.motion.price});console.log('CLIP',r.name,format)
  }
  if(families.includes(r))for(const [kind,frame] of [['intro',format==='vertical'?32:12],...(format==='horizontal'?[['intro-logo',Math.floor(scenes[1].from*.48)+15]]:[]),['outro',scenes.at(-1).from+40]]){

...
try{const report=JSON.parse(await readFile('output/video-studio-validation/voice-report.json','utf8'));const p=await api('/api/videos/projects/'+report.projectId);const job=await api('/api/videos/jobs','POST',{projectId:p.id,revision:p.revision,kind:'render'});console.log(JSON.stringify(job))}finally{await pool.end()}
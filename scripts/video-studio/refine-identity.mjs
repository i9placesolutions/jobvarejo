...
try {
 ...
 await writeFile(dir+'/identity-source.json',JSON.stringify({project:p,voice},null,2));
 ...
 if(process.argv.includes('--prepare')) {
   for(const [name,id] of [['identity-music.mp3',p.document.audio.music],...Object.entries(voice.result.clips).map(([id,c])=>['identity-'+id+'.mp3',c.assetId])]) {
 ...
 if(process.argv.includes('--render')) {
   ...
   await writeFile(dir+'/identity-render-job.json',JSON.stringify(render,null,2));
 ...
 if(process.argv.includes('--download')) {
   ...
   if(job?.status==='ready'){
     for(const {format,assetId} of job.result.outputs){
       ...
       const media=await fetch(r.headers.get('location'));if(!media.ok)throw Error('Download falhou');
       await writeFile(dir+'/identity-'+format+'.mp4',Buffer.from(await media.arrayBuffer()));
     }
     await writeFile(dir+'/identity-result.json',JSON.stringify(job,null,2));
   }
 }

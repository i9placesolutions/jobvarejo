...
try{
const project=JSON.parse(await readFile('output/video-studio-professional/project.json','utf8'));const jobs=await api('/api/videos/jobs?projectId='+project.id);const job=jobs.items.find(j=>j.kind==='render'&&j.revision===project.revision);console.log(JSON.stringify(job));
if(job?.status==='ready'){for(const {format,assetId} of job.result.outputs||[]){const r=await fetch(base+'/api/videos/assets/'+assetId,{headers:{Authorization:'Bearer '+token},redirect:'manual'});if(r.status!==302)throw Error('Asset indisponível');const media=await fetch(r.headers.get('location'));if(!media.ok)throw Error('Download falhou');await writeFile('output/video-studio-professional/'+format+'.mp4',Buffer.from(await media.arrayBuffer()))}await writeFile('output/video-studio-professional/result.json',JSON.stringify(job,null,2))}

...
try {
 ...
 await writeFile(dir+'/template-demo-source.json',JSON.stringify({project:p,voice},null,2));
 ...
 if(process.argv.includes('--prepare')) {
   for(const [name,id] of [['template-demo-music.mp3',p.document.audio.music],...Object.entries(voice.result.clips).map(([id,c])=>['template-demo-'+id+'.mp3',c.assetId])]) {
 ...
 if(process.argv.includes('--render')) {
   const d=p.document; d.validity='20 A 27/09/2026';d.voice.enabled=false;
   ...
   await writeFile(dir+'/template-demo-render-job.json',JSON.stringify(render,null,2));
 ...
 if(process.argv.includes('--download')) {
   ...
   if(job?.status==='ready'){
     for(const {format,assetId} of job.result.outputs){
       ...
       const media=await fetch(r.headers.get('location'));if(!media.ok)throw Error('Download falhou');
       await writeFile(dir+'/template-demo-'+format+'.mp4',Buffer.from(await media.arrayBuffer()));
     }
     await writeFile(dir+'/template-demo-result.json',JSON.stringify(job,null,2));
   }
 }

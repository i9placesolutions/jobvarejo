...
try{const r=await api('/api/label-templates');await writeFile('output/video-studio-professional/labels.json',JSON.stringify(r));console.log(r.templates.map(t=>({id:t.id,name:t.name,kind:t.kind,objects:t.group?.objects?.length})));}finally{await pool.end()}
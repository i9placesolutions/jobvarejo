...
// Famílias dirigidas manualmente conservam identidade, áudio e geometria ao regenerar o catálogo.
const custom=JSON.parse(await readFile('shared/video-studio/custom-flyer-recipes.json','utf8').catch(error=>{if(error.code==='ENOENT')return '[]';throw error}))
for(const recipe of custom){const index=recipes.findIndex(r=>r.sourceProject===recipe.sourceProject);if(index>=0)recipes[index]=recipe;else recipes.push(recipe)}

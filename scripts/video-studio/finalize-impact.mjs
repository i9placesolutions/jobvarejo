...
try{const p=await api('/api/videos/projects/107730ec-9bd7-4b66-ad57-1073bf2345ed');const jobs=await api('/api/videos/jobs?projectId='+p.id);console.log(JSON.stringify(jobs));if(process.argv.includes('--render')){const job=await api('/api/videos/jobs','POST',{projectId:p.id,revision:p.revision,kind:'render'});await writeFile('output/video-studio-impact/render-job.json',JSON.stringify(job));console.log(JSON.stringify(job))}}finally{await pool.end()}

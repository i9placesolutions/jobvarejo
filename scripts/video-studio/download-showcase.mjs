...
try{
const project=await api('/api/videos/projects/107730ec-9bd7-4b66-ad57-1073bf2345ed');const jobs=await api('/api/videos/jobs?projectId='+project.id);const job=jobs.items.find(j=>j.kind==='render'&&j.revision===project.revision);console.log(JSON.stringify(job));
if(job?.status==='ready'){for(const {format,assetId} of job.result.outputs||[]){const r=await fetch(base+'/api/videos/assets/'+assetId,{headers:{Authorization:'Bearer '+token},redirect:'manual'});if(r.status!==302)throw Error('Asset indisponível');const media=await fetch(r.headers.get('location'));if(!media.ok)throw Error('Download falhou');await writeFile('output/video-studio-impact/showcase-'+format+'.mp4',Buffer.from(await media.arrayBuffer()))}await writeFile('output/video-studio-impact/showcase-result.json',JSON.stringify(job,null,2))}

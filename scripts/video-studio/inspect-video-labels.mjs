...
try{const r=await api('/api/videos/labels');await writeFile('output/video-flyer-models/labels.json',JSON.stringify(r));console.log(r.items.map(t=>({id:t.id,name:t.name,width:t.width,height:t.height,nodes:t.nodes.map(n=>({name:n.name,type:n.type,fill:n.fill,src:n.src?'embedded':undefined}))})));}finally{await pool.end()}
...
const root='artifacts/art-studio/colecao-outubro',url='http://127.0.0.1:3119';
...
try{
 ...
 for(const name of Object.keys(sources)){
  ...
  const key=`imagens/biblioteca/elementos/colecao-outubro/${name}-${hash.slice(0,12)}.png`;
  ...
  const e=manifest.elements[name];await req('/api/asset-names',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({asset_key:e.key,display_name:'Coleção Outubro — '+name.replaceAll('-',' ')})});
 ...
 const library=await req('/api/assets?category=elementos&search=colecao-outubro&ai=0&limit=100');const found=new Set(library.map(x=>x.key));for(const e of Object.values(manifest.elements))if(!found.has(e.key))throw Error('Não listado na biblioteca '+e.key);

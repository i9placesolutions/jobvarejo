...
for(const model of CARTAZISTA_STARTER_MODELS){
  ...
  const edit=document.createElement('a');edit.textContent='Editar este modelo';edit.style.cssText='display:block;padding:12px 14px;color:#185db2;font-weight:700';edit.href=`http://127.0.0.1:4413/cartazista/editor/new?model=${encodeURIComponent(model.id)}&theme=${encodeURIComponent(paletteSelect.value)}&header=${encodeURIComponent(headerSelect.value)}`;card.append(edit)
  const header=headerSelect.value==='auto'?undefined:headers.find(h=>h.id===headerSelect.value)??null
  renderCartazistaSvg(cartazistaSample(model.id,headers,{themeId:paletteSelect.value as CartazistaThemeId,header})).then(svg=>{if(current===generation)card.querySelector('figure')!.innerHTML=svg}).catch(error=>{card.querySelector('figure')!.textContent=String(error);console.error(error)})
}
}
